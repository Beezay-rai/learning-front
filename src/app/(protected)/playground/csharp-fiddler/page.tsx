...
export default function CSharpFiddlerPage() {
    ...
    const handleRun = useCallback(async () => {
        ...
        try {
            ...
            setOutput(run.output || (run.stderr ? `Error: ${run.stderr}` : "No output received."));
            setStatus(run.code === 0 ? "success" : "error");
        } catch (err: any) {
            setOutput(`API Error: ${err.message || "Failed to execute code"}`);
            setStatus("error");
        } finally {
            setLoading(false);
        }
    ...
    return (
        <Box display="flex" flexDirection="column" height="calc(100vh - 110px)" gap={2}>
            {/* Header */}
            <Box display="flex" alignItems="center" justifyContent="space-between">
                <Box>
                    <Typography variant="h5" fontWeight={700}>
                        <TerminalIcon sx={{ verticalAlign: "middle", mr: 1, color: "#8b5cf6" }} />
                        C# Fiddler
                    </Typography>
            ...
            <Box display="flex" flex={1} gap={2} minHeight={0}>
                {/* Editor Panel */}
                <Box flex={1.5} display="flex" flexDirection="column" gap={0.5} minHeight={0}>
                    <Box display="flex" alignItems="center" justifyContent="space-between" px={0.5}>
                        <Typography variant="caption" fontWeight={700} color="text.secondary" textTransform="uppercase">
                            Code Editor
                        </Typography>
                        <Chip label="C# / .NET 6" size="small" variant="outlined" sx={{ fontSize: "0.65rem" }} />
                    </Box>
                    <Box
                        flex={1}
                        sx={{
                ...
                <Box flex={1} display="flex" flexDirection="column" gap={0.5} minHeight={0}>
                    <Box display="flex" alignItems="center" justifyContent="space-between" px={0.5}>
                        ...
                        <Stack direction="row" spacing={0.5}>
                            ...
                            <Tooltip title="Clear Console">
                                <IconButton size="small" onClick={handleClear} disabled={!output}>
                    ...
                    >
                        <Box component="pre" sx={{ m: 0, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
                            {output || <Typography variant="body2" color="text.disabled">Output will appear here...</Typography>}
                        </Box>
                    </Paper>
                </Box>
            </Box>
